<?php
namespace Pure\Requests\Templates\PersonalSettings{
    class Settings{
                ...
                if ((int)$current->ID === (int)$parameters->user) {
                    try{
                        ...
                        if (is_null($settings) === false){
                            ...
                            $BuddyPressPersonalSettings = new \Pure\Components\BuddyPress\PersonalSettings\User();
                            foreach($settings as $field_name=>$field_value){
                                if ($BuddyPressPersonalSettings->set((object)array(
                                        'user_id'   =>(int)$parameters->user,
                                        'field'     =>(string)$field_name,
                                        'value'     =>$field_value)) === false){
                                    $BuddyPressPersonalSettings = NULL;
                    ...
                    if (isset($_FILES['file']) === true){
                        if ($_FILES['file']['size'] < wp_max_upload_size()){
                            ...
                            if ($crop === false){
                                ...
                                $result = $PersonalSettings->setTitleImage(
                                    (object)array(
        ...
        public function delTitleImage($parameters){
            ...
            if ($this->validate($parameters, __METHOD__) === true) {
                ...
                if ((int)$current->ID === (int)$parameters->user) {
                    \Pure\Components\BuddyPress\PersonalSettings\Initialization::instance()->attach();
                    $PersonalSettings = new \Pure\Components\BuddyPress\PersonalSettings\User();
                    if ($PersonalSettings->deleteTitleImage((object)array('user_id'=>$parameters->user)) !== false){
                        $response->url      = '';
                        $response->message  = 'success';
                    }else{
