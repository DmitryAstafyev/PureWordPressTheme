    ...
    class Widget extends \WP_Widget {
        ...
        public function widget($args, $instance)
        {
            ...
            try{
                ...
                $ErrorMessages->show("\\Pure\\Plugins\\OpenMenu\\Widget\\widget::: ".$e);
                $ErrorMessages = NULL;
                return NULL;
            }
        }
        ...
        {
            $templates          = \Pure\Templates\Elements\OpenMenu\Initialization      ::instance()->templates;
            ...
            foreach ($templates as $_template){
                ...
                <p>
                    ...
                    <?php
                    \Pure\Templates\Elements\OpenMenu\Initialization::instance()->description($_template->key);
                    ?>
                </p>
            <?php
            }
            ...
            ?>
                <select class="widefat" id="<?php echo $this->get_field_id( 'target' ); ?>" name="<?php echo $this->get_field_name( 'target' ); ?>">
                    <option value="-1" <?php selected( -1, $target ); ?>><?php echo __('Hot links', 'pure'); ?></option>
                    <?php
                    if (is_array($menus) !== false){
                        foreach($menus as $menu){
                            ?>
                            <option value="<?php echo $menu->term_id;?>" <?php selected( $menu->term_id, $target ); ?>><?php echo $menu->name; ?></option>
                        <?php
                        }
                    }
                    ?>
