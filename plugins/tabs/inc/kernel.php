    ...
    class Builder{
        ...
        function __construct($parameters){
            ...
            if (is_bool($this->parameters) == true){
                throw new \Exception("Pure\Plugins\Tabs\Builder\__construct::: cannot validate settings of widget", E_USER_WARNING);
            }
        }
        private function validate($parameters){
            ...
            $parameters['captions'      ] = (isset($parameters['captions'       ]) == false ? false : (is_array($parameters['captions'      ]) !== false ? $parameters['captions'       ] : array()   ));
            $parameters['images'        ] = (isset($parameters['images'         ]) == false ? false : (is_array($parameters['images'        ]) !== false ? $parameters['images'         ] : array()   ));
        ...
        private function show(){
            ...
            if (count($this->parameters['icons']) > 0){
                $template   = \Pure\Templates\PresentationTabs\Initialization::instance()->get($this->parameters['template']);
                if ($template !== false){
                    $items      = array();
                    for($index = 0; $index < count($this->parameters['icons']); $index ++){
                        ...
                        $image      = wp_get_attachment_image_src($this->parameters['images'][$index], 'full');
                        $image      = (is_array($image) !== false ? $image[0] : '');
                        $items[]    = (object)array(
                            'caption'       =>$this->parameters['captions'      ][$index],
                            'image'         =>$image,
