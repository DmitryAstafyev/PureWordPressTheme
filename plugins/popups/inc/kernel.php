    ...
    class Builder{
        ...
        function __construct($parameters){
            ...
            if (is_bool($this->parameters) == true){
                throw new \Exception("Pure\Plugins\Popups\Builder\__construct::: cannot validate settings of widget", E_USER_WARNING);
            }
        }
        private function validate($parameters){
            ...
            $parameters['subcaptions'   ] = (isset($parameters['subcaptions'    ]) == false ? false : (is_array($parameters['subcaptions'   ]) !== false ? $parameters['subcaptions'    ] : array()   ));
        ...
        private function show(){
            ...
            if (count($this->parameters['icons']) > 0){
                $template   = \Pure\Templates\Popups\Initialization::instance()->get($this->parameters['template']);
                if ($template !== false){
                    $items      = array();
                    for($index = 0; $index < count($this->parameters['icons']); $index ++){
                        ...
                        $items[]    = (object)array(
                            ...
                            'subcaption'    =>$this->parameters['subcaptions'   ][$index],
