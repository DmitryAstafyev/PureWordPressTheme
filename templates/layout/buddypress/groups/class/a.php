<?php
namespace Pure\Templates\Layout\BuddyPress\Groups{
    class A{
        ...
        private function innerHTMLFriendsGroups($user_id, $member_full){
            ...
            if ($friends_IDs !== false){
                if (count($friends_IDs) > 0){
                    ...
                    $innerHTMLFriendsGroups = $groups->render();
                    $innerHTMLFriendsGroups = ($innerHTMLFriendsGroups !== '' ? $innerHTMLFriendsGroups : '<p data-element-type="Pure.Social.Home.A.Message">'.$member_full->author->name.' '.__('are not a member of any group yet.', 'pure').'</p>');
                }
            }
            return $innerHTMLFriendsGroups;
        }
        private function innerHTMLGroups($user_id, $member_full){
            $groups                 = new \Pure\Plugins\Thumbnails\Groups\Builder(array(
                'content'           => 'users',
                'targets'	        => (int)$user_id,
                'template'	        => 'J',
        ...
        private function innerHTMLBlock($user_id, $group_IDs, $container_id, $post_type){
            switch($post_type){
                case 'post':
                    $template   = 'F';
        ...
        private function innerHTMLContent($user_id, $groups_IDs, $member_full){
            if (is_array($groups_IDs) !== false){
                if (count($groups_IDs) > 0){
                    ...
                    $counts                 = $PostsProvider->get_groups_posts_counts_by_types($groups_IDs);
                    ...
                    $innerHTMLCounter       = Initialization::instance()->html(
                        ...
                        array(
                            array('title',      __('IN GROUPS OF '.$member_full->author->name.' WRITES', 'pure') ),
                    ...
                    foreach($labelsIDs as $key=>$value){
                        $innerHTMLTabs .= Initialization::instance()->html(
                            ...
                            array(
                                ...
                                array('content',        $this->innerHTMLBlock((int)$user_id, $groups_IDs, $value, $key)   ),
                            )
                        );
                    }
        ...
        public function get($member){
            ...
            $groups_IDs             = $GroupsCommon->get_groups_IDs_where_user_is((int)$member->id);
            $GroupsCommon           = NULL;
            ...
            $innerHTMLGroups        = Initialization::instance()->html(
                'A/one_column_segment_central',
                array(
                    array('title',      $member_full->author->name.' '.__('are a member of', 'pure') ),
                    array('content',    $this->innerHTMLGroups((int)$member->id, $member_full)              ),
                )
            );
            $innerHTMLFriendsGroups = Initialization::instance()->html(
                'A/one_column_segment_central',
                array(
            ...
            $innerHTML              = Initialization::instance()->html(
                ...
                array(
                    ...
                    array('posts',              $this->innerHTMLContent((int)$member->id, $groups_IDs, $member_full)        ),
                    array('post_count',         $innerHTMLPostCount                                                         ),
                    array('groups',             $innerHTMLGroups                                                            ),
                    array('friends_groups',     $innerHTMLFriendsGroups                                                     ),
