<?php
namespace Pure\Templates\Layout\WordPress\Tag{
    class A{
        ...
        private function innerHTMLBlock($container_id, $post_type, $tag_id){
            switch($post_type){
                case 'post':
                    $template   = 'F';
        ...
        public function get($tag_id){
            ...
            $counts                 = $PostsProvider->get_posts_counts_by_types(false, false, $tag_id);
            ...
            $innerHTMLTitle         = Initialization::instance()->html(
                'A/about',
                array(
                    array('name',   $tag[0]->name                               ),
                    array('info',   __('all post of tag', 'pure')   ),
                )
            );
            ...
            foreach($labelsIDs as $key=>$value){
                $innerHTMLTabs .= Initialization::instance()->html(
                    ...
                    array(
                        ...
                        array('content',        $this->innerHTMLBlock($value, $key, $tag_id)),
                    )
                );
            }
