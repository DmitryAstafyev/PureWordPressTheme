<?php
namespace Pure\Templates\Pages\ReportEditor{
    class A{
        private $id;
        private $current;
        ...
        private function checkPermissions(){
            ...
            if ($this->current->mana !== false){
                if ($this->current->mana->allow_create->report === false){
                    $innerHTML = $this->GroupWrapper->open(array(
                            "title"             =>__( "You cannot create report", 'pure' ),
                            "group"             =>uniqid(),
                            "echo"              =>false,
                            "opened"            =>true,
                    ...
                    $innerHTML .=   '<p data-type-element="Pure.PostEditor.A.Notice">'.
                                        __( "Sorry, but you cannot create report, because you have not enough mana (rate) for it. You should have at least ", 'pure' ).
                                        '<strong style="color:green;">('.$this->current->mana->threshold->allow_create->report.')</strong>'.
        ...
        private function elementTitle(){
            if ($this->post !== false){
                $innerHTML = '<p data-type-element="Pure.PostEditor.A.Title">'.__( "Edit the report", 'pure' ).'</p>';
            }else{
                $innerHTML = '<p data-type-element="Pure.PostEditor.A.Title">'.__( "Create a new report", 'pure' ).'</p>';
            }
            return $innerHTML;
        }
        private function elementDetails(){
            ...
            if ($this->post !== false){
                if ($this->post->post->post_status !== 'publish'){
                    $innerHTML = '<p data-type-element="Pure.PostEditor.A.Attention">'.__( "This report is not published yet.", 'pure' ).'</p>';
                }
        ...
        private function elementHEADER(){
            $innerHTML = $this->GroupWrapper->open(array(
                    "title"             =>__( "Name of your report", 'pure' ),
                    "group"             =>uniqid(),
                    "echo"              =>false,
                    "opened"            =>true,
                    "content_style"     =>'width:auto;padding:0.5em;')
            ...
            $innerHTML .= '<p data-type-element="Pure.PostEditor.A.Notice">'.__( "Please define name of your report. Not more 250 symbols.", 'pure' ).'</p>';
            $innerHTML .= $this->GroupWrapper->close(array("echo"=>false));
            return $innerHTML;
        }
        ...
        private function elementINDEXES(){
            $innerHTML = $this->GroupWrapper->open(array(
                    "title"             =>__( "Setup indexes for your report", 'pure' ),
                    "group"             =>uniqid(),
                    "echo"              =>false,
                    "opened"            =>true,
                    "content_style"     =>'width:auto;padding:0.5em;')
            ...
            $collections    = $Provider->getAvailableCollections();
            if ($this->post !== false){
                $report_collection = $Provider->get($this->post->post->id);
            }else{
                $report_collection = false;
            }
            $Provider       = NULL;
            $innerHTML .=   '<p data-type-element="Pure.PostEditor.A.SubTitle">'.__( "You should define collection of indexes to rate results of your report. All registered users will vote by your indexes too.", 'pure' ).'</p>';
            if ($report_collection === false){
                $innerHTML .=   '<p data-type-element="Pure.PostEditor.A.Notice">'.__( "Please, remember, you cad define indexes only once. You can vote, after report will be published.", 'pure' ).'</p>';
                $innerHTML .=   '<table data-type-element="Pure.PostEditor.A.Collection">';
                foreach($collections as $collection_key=>$collection){
                    $innerHTMLIndexes   = '<ol data-type-element="Pure.PostEditor.A.Indexes">';
                    $innerHTMLMaximums  = '<ol data-type-element="Pure.PostEditor.A.Maximums">';
                    foreach($collection->indexes as $index){
                        $innerHTMLIndexes   .= '<li>'.$index->name.'</li>';
                        $innerHTMLMaximums  .= '<li>'.$index->max.'</li>';
                    }
                    $innerHTMLIndexes   .= '</ol>';
                    $innerHTMLMaximums  .= '</ol>';
                    $innerHTML .=   '<tr>'.
                                        '<td>'.
                                            '<label>'.
                                                '<input form="'.$this->id.'" data-element-type="Pure.CommonStyles.CheckBox.A" type="radio" name="report_collection" value="'.$collection_key.'"/>'.
                ...
                $innerHTML .=   '<p data-type-element="Pure.PostEditor.A.Notice">'.__( "You cannot change collection of indexes after report was published.", 'pure' ).'</p>';
                $innerHTML .=   '<ol data-type-element="Pure.PostEditor.A.Indexes">';
                foreach($report_collection->indexes as $key=>$index){
                    $innerHTML .=   '<li>'.$index.'</li>';
                }
                $innerHTML .=   '</ol>';
                $innerHTML .=   '<input form="'.$this->id.'" data-element-type="Pure.CommonStyles.CheckBox.A" type="checkbox" checked name="report_collection" value="-1"/>';
            }
            $innerHTML .=   $this->GroupWrapper->close(array("echo"=>false));
            return $innerHTML;
        }
        private function elementPLACE(){
            ...
            $map        = $Reports->getPlace($id);
            $Reports    = NULL;
            $innerHTML  = $this->GroupWrapper->open(array(
                    "title"             =>__( "You can define place for your report.", 'pure' ),
                    "group"             =>uniqid(),
                    "echo"              =>false,
                    "opened"            =>true,
                    "content_style"     =>'width:auto;padding:0.5em;')
            ...
            $innerHTML .=   '<div data-type-element="Pure.PostEditor.A.MapContainer" '.
                                ...
                                'data-google-maps-engine-address="'.$map->on_map.'" '.
                            ...
                            '<div data-type-element="Pure.PostEditor.A.Field">'.
                                '<input '.
                                    ...
                                    'form="'.$this->id.'" '.
                                    'name="report_place_name" '.
                                    'value="'.($this->post !== false ? $map->place : '').'" '.
                                '/>'.
                            '</div>'.
                            '<p data-type-element="Pure.PostEditor.A.Notice">'.__( "Write here name of place or how you want to call place of report about is.", 'pure' ).'</p>';
            $innerHTML .= $this->GroupWrapper->close(array("echo"=>false));
            return $innerHTML;
        }
        private function elementQUOTE(){
            $innerHTML = $this->GroupWrapper->open(array(
                    "title"             =>__( "Short description of your report", 'pure' ),
                    "group"             =>uniqid(),
                    "echo"              =>false,
                    "opened"            =>false,
                    "content_style"     =>'width:auto;padding:0.5em;')
            ...
            $innerHTML .= '<p data-type-element="Pure.PostEditor.A.Notice">'.__( "You can define short description for your report. This is not a mandatory field. If you do not define it, quote will be built by content of your report. Not more 500 symbols.", 'pure' ).'</p>';
            $innerHTML .= $this->GroupWrapper->close(array("echo"=>false));
            return $innerHTML;
        }
        private function elementMINIATURE(){
            ...
            $innerHTML = $this->GroupWrapper->open(array(
                    "title"             =>__( "Miniature for your report", 'pure' ),
                    "group"             =>uniqid(),
                    "echo"              =>false,
                    "opened"            =>false,
                    "content_style"     =>'width:auto;padding:0.5em;')
            ...
            $innerHTML .=   '<p data-type-element="Pure.PostEditor.A.Notice">'.__( "You can define some image for your report. This is not a mandatory field. If your report has some images, it will be used as miniature.", 'pure' ).'</p>';
            $innerHTML .= $this->GroupWrapper->close(array("echo"=>false));
            return $innerHTML;
        }
        private function elementVISIBILITY(){
            ...
            $GroupProvider  = NULL;
            $innerHTML = $this->GroupWrapper->open(array(
                    "title"             =>__( "Visibility of your report", 'pure' ),
                    "group"             =>uniqid(),
                    "echo"              =>false,
                    "opened"            =>false,
                    "content_style"     =>'width:auto;padding:0.5em;')
            ...
            $innerHTML .=   '<table data-type-element="Pure.PostEditor.A.Visibility">'.
                                '<tr>'.
                                    ...
                                    '</td>'.
                                    '<td>'.
                                        '<p data-type-element="Pure.PostEditor.A.Visibility">'.__( "Your report will be available for all user. And for registered and not.", 'pure' ).'</p>'.
                                    '</td>'.
                                '</tr>'.
                                '<tr>'.
                                    '<td>'.
                                    ...
                                    '</td>'.
                                    '<td>'.
                                        '<p data-type-element="Pure.PostEditor.A.Visibility">'.__( "Show report only registered users.", 'pure' ).'</p>'.
                                    '</td>'.
                                '</tr>'.
                            '</table>';
            $innerHTML .=   '<p data-type-element="Pure.PostEditor.A.SubTitle"><strong>'.__( "Second level.", 'pure' ).'</strong> '.__( "You can define second level of visibility of your report.", 'pure' ).'</p>';
            $innerHTML .=   '<table data-type-element="Pure.PostEditor.A.Visibility">'.
                                '<tr>'.
                                    '<td>'.
                                        '<label>'.
                                    ...
                                    '</td>'.
                                    '<td>'.
                                        '<p data-type-element="Pure.PostEditor.A.Visibility">'.__( "With this setting of visibility your report will be available for any user according a first level of visibility.", 'pure' ).'</p>'.
                                    '</td>'.
                                '</tr>'.
                                '<tr>'.
                                    '<td>'.
                                    ...
                                    '</td>'.
                                    '<td>'.
                                        '<p data-type-element="Pure.PostEditor.A.Visibility">'.__( "Use this setting of visibility to associate report with your friends. If first level of visibility is \"registered\", your report will see only your friends.", 'pure' ).'</p>'.
                                    '</td>'.
                                '</tr>';
            if ($groups !== false){
                if (count($groups->groups) > 0){
                    $innerHTML .=   '<tr>'.
                                        ...
                                        '</td>'.
                                        '<td>'.
                                            '<p data-type-element="Pure.PostEditor.A.Visibility">'.__( "Use this setting of visibility to associate report with some group, where you are a member. If first level of visibility is \"registered\", your report will see only members of selected group.", 'pure' ).'</p>'.
        ...
        private function elementCATEGORIES(){
            ...
            if (is_array($categories) !== false){
                if (count($categories) > 0){
                    $innerHTML = $this->GroupWrapper->open(array(
                            "title"             =>__( "Define category of your report", 'pure' ),
                            "group"             =>uniqid(),
                            "echo"              =>false,
                            "opened"            =>false,
                            "content_style"     =>'width:auto;padding:0.5em;')
                            ...
                            if ($sandbox !== false){
                                ...
                                if ($this->current->mana->allow_manage->categories === false){
                                    $innerHTML .=   '<p data-type-element="Pure.PostEditor.A.Attention">'.
                                                        ...
                                                        __( " but you have ", 'pure' ).
                                                        '<strong style="color:red;">('.$this->current->mana->value.')</strong>.'.
                                                        __( " If you are updating report, updated report will be placed into sandbox too.", 'pure' ).
                                                    '</p>';
                                }
                            }
                        }
                    }
                    $innerHTML .=   '<p data-type-element="Pure.PostEditor.A.Notice">'.__( "It will be better, if you define category of your report.", 'pure' ).'</p>';
                    $innerHTML .= $this->GroupWrapper->close(array("echo"=>false));
                }
            }
            return $innerHTML;
        ...
        private function elementSETTINGS(){
            ...
            if ($allow_manage === false){
                ...
                $innerHTML .=   '<p data-type-element="Pure.PostEditor.A.Notice">'.__( "You can allow or deny comments to your report.", 'pure' ).'</p>';
            }
            $innerHTML .= $this->GroupWrapper->close(array("echo"=>false));
            return $innerHTML;
        }
        ...
        private function errorInnerHTML($error){
            switch($error){
                case 'report not found':
                    $innerHTML = $this->GroupWrapper->open(array(
                            "title"             =>__( "Report not found", 'pure' ),
                            "group"             =>uniqid(),
                            "echo"              =>false,
                            "opened"            =>true,
                            "content_style"     =>'width:auto;padding:0.5em;')
                    ...
                    $innerHTML .=   '<p data-type-element="Pure.PostEditor.A.Attention">'.
                                        __( "Sorry, but report, which you are required, was not found. Please check url and try again.", 'pure' ).
                                    '</p>';
                    $innerHTML .= $this->GroupWrapper->close(array("echo"=>false));
                    return $innerHTML;
                    break;
                case 'access deny':
                    ...
                    $innerHTML .=   '<p data-type-element="Pure.PostEditor.A.Attention">'.
                        __( "Sorry, but to create report you should register first.", 'pure' ).
                        '</p>';
                    $innerHTML .= $this->GroupWrapper->close(array("echo"=>false));
                    return $innerHTML;
                    break;
        ...
        private function setPostID($current){
            ...
            if ($this->post !== false){
                ...
                $post_ID    = $Provider->addEmptyDraft('report');
                $Provider   = NULL;
        ...
        public function get($post_id = false){
            ...
            if ($current !== false){
                ...
                if ($post_id === false){
                    ...
                    $createURL  = $Data->url;
                    $command    = 'templates_of_reports_create_report';
                    $Data       = NULL;
                    if ($this->setPostID($current) !== false){
                        return $this->innerHTML($createURL, $command);
                    }else{
                        return $this->errorInnerHTML('Sorry, some error on server. Cannot create draft for new report.');
                    }
                }else{
                    if ($this->getPostData($post_id) !== false){
                        if ($this->post->post->post_type === 'report'){
                            $Data       = new \Pure\Components\WordPress\Location\Requests\Register();
                            $createURL  = $Data->url;
                            $command    = 'templates_of_reports_update_report';
                            $Data       = NULL;
                            if ($this->setPostID($current) !== false){
                                return $this->innerHTML($createURL, $command);
                            }else{
                            ...
                            return $this->errorInnerHTML('report not found');
                        }
                    }else{
                        return $this->errorInnerHTML('report not found');
                    }
