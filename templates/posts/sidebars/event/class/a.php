<?php
namespace Pure\Templates\Posts\Sidebars\Event{
    class A{
        ...
        private function innerHTMLAuthorFriends($user_id){
        ...
        public function innerHTML($parameters){
            ...
            if ($this->validate($parameters) !== false){
                $this->getSettings();
                $user_id    = (int)$parameters->post->post_author;
                $WordPress  = new \Pure\Components\WordPress\UserData\Data();
                $user_name  = $WordPress->get_name($user_id);
                $WordPress  = NULL;
                $innerHTML .=   '<!--BEGIN: Post.Sidebar.A -->'.
                                '<div data-post-element-type="Pure.Posts.SideBar.A.Container">'.
                                        $this->innerHTMLTitle(__('More from author', 'pure')).
                                        $this->innerHTMLAuthorPosts($user_id).
                                        $this->innerHTMLTitle($user_name.' '.__('friends', 'pure')).
                                        $this->innerHTMLAuthorFriends($user_id).
                                '</div>'.
                                '<!--END: Post.Sidebar.A -->';
            }
